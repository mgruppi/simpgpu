...
bool Surface::collapse(Edge& e)
{
    ...
    //cerr << "Collapse: " << e.p1->id << "-" << e.p2->id << endl;

    //Iterating faces
  vector<Face*> for_removal;
  vector<Face*> check_intersection; //Modified faces must to be checked for intersection.
  //clock_gettime(CLOCK_REALTIME,&t0);
    for(vector<Face*>::iterator fit = p1->faces.begin(); fit!=p1->faces.end(); ++fit)
    {
        ...
        if(removeface) //Faces sharing both vertices become degenerate.
        {
        ...
        else //Swap p1 for p2 for this face
        {
          ...
          //(*auxp1)->x = e.placement->x; (*auxp1)->y = e.placement->y; (*auxp1)->z = e.placement->z;
          //TODO: keep p1 for reverting collapse in case of intersection
          (*auxp1) = p2;
          p2->faces.push_back((*fit));
          check_intersection.push_back(*fit);

        }
    }

    //Set position of p2 as placement of e between p1-p2(default: midpoint p1-p2)
    p2->x = e.placement->x;
    p2->y = e.placement->y;
    p2->z = e.placement->z;
    ...
    // for(face_vec_it fit = check_intersection.begin(); fit != check_intersection.end(); ++fit)
    // {
    //
    //   //Face has been modified. Checking intersection.
    ...
    //
    //   Triangle3 face(p30,p31,p32); //Triangle out of face *fit
    //
    //   //Brute force: test against all other faces
    //   for(face_vec_it fit2 = m_faces.begin(); fit2 != m_faces.end(); ++fit2)
    //   {
    //
    //     bool share_vertex = false;
    //     Face* f = (*fit2);
    //     if(f->id == (*fit)->id || f->removed) continue;
    //     //Check whether the two faces have vertices in common
    //     for(point_vec_it pit = (*fit)->points.begin(); pit!=(*fit)->points.end();++pit)
    //     {
    //       for(point_vec_it pit2 = f->points.begin(); pit2!=f->points.end();++pit2)
    //       {
    //
    //         if((*pit)->id == (*pit2)->id){
    //           //cerr << "Faces " << (*fit)->id << " and " << f->id << " share a vertex.\n";
    //           share_vertex = true;
    //           break;
    //
    //         }
    //       }
    //       if(share_vertex) break;
    //     }
    //
    //     //Both faces share a vertex, don't check for intersections
    //     if(!share_vertex){
    //
    ...
    //         cerr << "("<<(*fit)->points[0]->id << " " <<(*fit)->points[1]->id << " " << (*fit)->points[2]->id << ")\n";
    //         cerr << "("<<f->points[0]->id << " " <<f->points[1]->id << " " << f->points[2]->id << ")\n";
    //         for(face_vec_it ff=for_removal.begin(); ff != for_removal.end(); ++ff)
    //         {
    //           if((*ff)->id == f->id) cerr << "yes\n";
    //         }
    //       }
    //     }
    //   }
